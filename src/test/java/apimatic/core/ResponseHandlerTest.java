...
public class ResponseHandlerTest extends MockCoreConfig {
    ...
    /**
     * Mock of {@link EndpointConfiguration}.
     */
    @Mock
    private EndpointConfiguration endpointSetting;
    ...
    private void prepareMockEndpointConfiguration() {
        // stubs
        when(endpointSetting.getGlobalConfiguration()).thenReturn(getMockGlobalConfig());
    }

    @Test
    public void testDeserializerMethod() throws IOException, CoreApiException {
        ResponseHandler<String, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<String, CoreApiException>()
        ...
        // verify
        assertEquals(coreResponseHandler.handle(getCoreHttpRequest(), coreHttpResponse, endpointSetting), "bodyValue");
    }

    @Test
    public void testDeserializerMethodUsingRawBody() throws IOException, CoreApiException {
        ResponseHandler<InputStream, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<InputStream, CoreApiException>()
                .build();
        ...
        // verify
        assertEquals(coreResponseHandler.handle(getCoreHttpRequest(), coreHttpResponse, endpointSetting), inputStream);
    }

    @Test
    public void testDeserializerMethodUsingContext() throws IOException, CoreApiException {
        ResponseHandler<TestModel, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<TestModel, CoreApiException>()
                .deserializer(string -> CoreHelper.deserialize(string, TestModel.class))
                .contextInitializer((context, response) -> response.toBuilder().httpContext(context).build()).build();
        ...
        // verify
        assertEquals(coreResponseHandler.handle(getCoreHttpRequest(), coreHttpResponse, endpointSetting).getContext(),
                model.getContext());
    }
    ...
    public void testContextWithoutDeserializer() throws IOException, CoreApiException {
        ...
        // verify
        assertNull(coreResponseHandler.handle(getCoreHttpRequest(), coreHttpResponse, endpointSetting));
    }

    @Test
    public void testDynamicResponseTypeMethod() throws IOException, CoreApiException {
        ResponseHandler<DynamicType, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<DynamicType, CoreApiException>()
        ...
        // verify
        assertEquals(coreResponseHandler.handle(getCoreHttpRequest(), coreHttpResponse, endpointSetting), dynamicType);
    }

    @Test
    public void testApiResponseTypeMethod() throws IOException, CoreApiException {
        ResponseHandler<ApiResponseType<String>, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<ApiResponseType<String>, CoreApiException>()
        ...
        // verify
        assertEquals(coreResponseHandler.handle(getCoreHttpRequest(), coreHttpResponse, endpointSetting).getResult(),
                stringApiResponseType.getResult());
    }

    @Test
    public void testDynamicApiResponseTypeMethod() throws IOException, CoreApiException {
        ResponseHandler<ApiResponseType<DynamicType>, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<ApiResponseType<DynamicType>, CoreApiException>()
    ...
    public void testNullify404() throws IOException, CoreApiException {
        ResponseHandler<String, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<String, CoreApiException>()
                .nullify404(true).build();
        // stub
        when(coreHttpResponse.getStatusCode()).thenReturn(NOT_FOUND_STATUS_CODE);
        when(getCompatibilityFactory().createHttpContext(getCoreHttpRequest(), coreHttpResponse)).thenReturn(context);
    ...
    public void testNullify404False() throws IOException, CoreApiException {
        ResponseHandler<String, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<String, CoreApiException>()
                .nullify404(false).globalErrorCase(getGlobalErrorCases()).build();
        // stub
        when(coreHttpResponse.getStatusCode()).thenReturn(NOT_FOUND_STATUS_CODE);

        CoreApiException apiException = assertThrows(CoreApiException.class, () -> {
            coreResponseHandler.handle(getCoreHttpRequest(), coreHttpResponse, endpointSetting);
        });
    ...
    public void testNullableResponseTypeFalse() throws IOException, CoreApiException {
        ResponseHandler<Integer, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<Integer, CoreApiException>()
                .nullableResponseType(false).deserializer(response -> Integer.parseInt(response))
        ...
        // verify
        assertEquals(Integer.valueOf(body),
                coreResponseHandler.handle(getCoreHttpRequest(), coreHttpResponse, endpointSetting));
    ...
    public void testLocalException() throws IOException, CoreApiException {
        ResponseHandler<String, CoreApiException> coreResponseHandler = new ResponseHandler.Builder<String, CoreApiException>()
                .localErrorCase("403",
                        ErrorCase.setReason("Forbidden", (reason, context) -> new CoreApiException(reason, context)))
                .build();
        // stub
    ...
    public void testGlobalException() throws IOException, CoreApiException {
        ...
        String exceptionResponse = "{\"ServerMessage\" : \"Internal server error\" , \"ServerCode\" : 500 }";
    ...
    private Map<String, ErrorCase<CoreApiException>> getGlobalErrorCases() {
        ...
        globalErrorCase.put("400",
                ErrorCase.setReason("Bad Request", (reason, context) -> new GlobalTestException(reason, context)));

        globalErrorCase.put("404",
                ErrorCase.setReason("Not found", (reason, context) -> new CoreApiException(reason, context)));

        globalErrorCase.put(ErrorCase.DEFAULT,
                ErrorCase.setReason("Invalid response.", (reason, context) -> new CoreApiException(reason, context)));
    ...
    private void prepareCompatibilityStub() {
        ...
        when(getCompatibilityFactory().createHttpRequest(any(Method.class), any(StringBuilder.class),
                any(HttpHeaders.class), anyMap(), any(Object.class))).thenReturn(getCoreHttpRequest());
        when(getCompatibilityFactory().createHttpRequest(any(Method.class), any(StringBuilder.class),
                any(HttpHeaders.class), anyMap(), anyList())).thenReturn(getCoreHttpRequest());
        when(getCompatibilityFactory().createHttpContext(getCoreHttpRequest(), coreHttpResponse)).thenReturn(context);

        when(getCompatibilityFactory().createDynamicResponse(coreHttpResponse)).thenReturn(dynamicType);

        when(getCompatibilityFactory().createApiResponse(any(int.class), any(HttpHeaders.class), any(String.class)))
                .thenReturn(stringApiResponseType);
